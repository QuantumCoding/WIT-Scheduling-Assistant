...
public class DepartmentClassesPage extends Page {
	...
	protected DepartmentClassesPage(Document doc, Choise term, Choise department, boolean... skipListing) {
		super(doc, term, department, skipListing.length > 0 ? skipListing[0] : false);
	}

	protected void init(Object[] args) {
		if((Boolean) args[2]) { doneLoading(); return; }
		...
		do {
			...
			while((bodyIndex = getNext(tableNodes, "tBody", bodyIndex)) != -1) {
				...
				while((roxIndex = getNext(bodyNodes, "tr", roxIndex)) != -1) {
					...
					classList.add(new ClassOption((Choise) args[0], (Choise) args[1], 
							data.get(1), Integer.parseInt(data.get(0)), data.get(2)));
				}
			}
