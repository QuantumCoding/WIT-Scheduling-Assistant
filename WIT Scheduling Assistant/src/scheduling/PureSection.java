...
import pages_java.ClassOption;

public class PureSection {
	...
	private ArrayList<PureSection> labs;
	
	public PureSection(PureSection prevSection, ClassOption subject, Element row) {
	...
	public static PureSection parse(ClassOption subject, String className, String classId, int courseNumber, String sectionId,
			double credits, ArrayList<Designation> designations, String instructor, boolean isLab, ArrayList<PureSection> labs) {
		PureSection section = new PureSection();
	...
	public ArrayList<PureClassConfig> findValidConfigurations(PureSection other) {
		ArrayList<PureClassConfig> validConfigs = new ArrayList<>();
		ArrayList<PureClassConfig> otherConfigs = new ArrayList<>();
		
		for(int i = -1; i < (labs == null ? 0 : labs.size()); i ++) {
			if(i > -1 && !labs.get(i).isViable()) continue;
			validConfigs.add(new PureClassConfig(this, i));
		}
		
		for(int i = -1; i < (other.labs == null ? 0 : other.labs.size()); i ++) {
			if(i > -1 && !other.labs.get(i).isViable()) continue;
			otherConfigs.add(new PureClassConfig(other, i));
		}
		
		for(PureClassConfig otherConfig : otherConfigs) {
			ArrayList<Designation> otherDesignations = otherConfig.getLab() == PureClassConfig.NO_LAB ? 
					other.designations : other.getLabs().get(otherConfig.getLab()).designations;
			
			ListIterator<PureClassConfig> iter = validConfigs.listIterator();
			
			collisionsLoop:
			while(iter.hasNext()) {
				PureClassConfig config = iter.next();
				ArrayList<Designation> currentDesignations = config.getLab() == PureClassConfig.NO_LAB ? 
		...
		if(hasLab && !validConfigs.isEmpty()) {
			if(validConfigs.get(0).getLab() == PureClassConfig.NO_LAB)
	...
	public ArrayList<PureSection> getLabs() { return labs; }
}
